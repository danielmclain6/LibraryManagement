...
public class BookCheckoutDaoImp implements BookCheckoutDao {
	...
	private static final String CHECKOUT_BOOK = "insert into book_checkout(patron_id, isbn, checkedout, due_date) "
			+ "values(?, ?, ?, ?) ";
	private static final String RETURN_BOOK = "update book_checkout set returned = ? where "
			+ "isbn = ?";
	...
	/*
	 * Takes a patron object, plus a book object. 
	 * Inserts an entry into the book_checkout table with current date as checkout date
	 * Also updates book table to show book as rented
	 * Due date is 30 days from current date
	 * 
	 * 
	 */
	...
	/*
	 * Takes a patron object, plus a String isbn. 
	 * Inserts an entry into the book_checkout table with current date as checkout date
	 * Due date is 30 days from current date
	 * Also updates the book in the book table to be shown as rented
	 * 
	 * 
	 */
	...
	/*
	 * Takes a Book parameter
	 * Sets rented status on both book and book_checkout tables as 0
	 * Plus sets the current date as the return date on book_checkout
	 * 
	 * 
	 * 
	 */
	...
	/*
	 * Takes isbn as parameter
	 * Sets return_date on book_checkout as current date
	...
	/*
	 * Takes patron and book as parameter
	 * Sets rented status to 0 on book table and sets returnDate on book_checkout as current date
	 * using isbn of book, plus the patron's ID
	 * 
	 */
		...
		try (PreparedStatement state = conn.prepareStatement(GET_BOOK_HISTORY)) {
			...
			while(rs.next()) {
				...
				if(rs.getString("returned") != null) {
					returnDate = rs.getString("returned");
				} else {
					returnDate = "Not Returned";
				}
				System.out.println(returnDate);
				BookCheckout bc = new BookCheckout(checkoutId, patronId, isbn, checkedOut, dueDate, returnDate);
				System.out.println(bc);
				bookHistory.add(new BookCheckout(checkoutId, patronId, isbn, checkedOut, dueDate, returnDate));
			}
